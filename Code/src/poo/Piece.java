...
public class Piece {
	...
	public boolean placerPiece(int x, int y) {
		...
		if (verifierPlacementPiece(x, y)) {
			...
			posX = x;
			posY = y;
			System.err.println("Plac�e en  : " + posX + " ; " + posY);
			jeu.afficherPlateau();
			return true;
		} else {
			enlever();
	...
	private boolean verifierPlacementPiece(int x, int y) {
		// Verification que les coordonn�es sont bien dans le plateau
		// if ((x > 3) || (x < 0) || (y > 3) || (y < 0)) {
		// return false;
		// }
		// Verification que la piece peut etre pos�e
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
