...
public class Piece {
	...
	// Contexte dans lequel elle a �t� plac�
	private Contexte contexte;
	private TypePiece typePiece;
	private int degreRotation;
	private boolean contenirCochon; // Servira pour les propri�t�s du mode nocturne
	...
	public Piece(TypePiece type) {
		...
		switch (type) {
		case Paille: // Cr�e une pi�ce avec la maison de paille
			...
			coordMaison.put("y", 1);
			break;
		case Bois: // Cr�e une pi�ce avec la maison de bois
			...
			coordMaison.put("y", 1);
			break;
		case Brique: // Cr�e une pi�ce avec la maison de brique
			piece[1][2] = TypeCase.Jardin;
	...
	public void tournerHoraire() {
		...
		for (int i = 0; i <= 2; i++) { // Parcourt le tableau de la pi�ce
			for (int j = 0; j <= 2; j++) { //
				if (piece[i][j] != null) {
					if (i == 0) {
	...
	public boolean verifPlacement(int x, int y, Contexte contexte) { 
		for (int i = 0; i < 3; i++) {
			for (int j = 0; j < 3; j++) {
				...
				// Si la piece est un jardin et que la case sur laquelle elle doit etre plac�
				...
				if (piece[i][j] == TypeCase.Jardin) {
					...
					if (Jeux.getPlateau()[x + i - coordMaison.get("x")][y + j - coordMaison.get("y")] != TypeCase.Vide) // A
																														// r�viser
						return false;
				}
			...
			/*	// Si la piece est une Maison et que la case sur laquelle elle doit etre placee est un cochon
				//on retourne true
				if (contexte==contexte.Nocturne) {
					if (piece[i][j] == TypeCase.Maison && Jeux.getPlateau()[x][y] != TypeCase.Cochon) {
						return true;
					}
				}
				*/
	...
	public void enlever() {
		if (posX != null && posY != null) {
			...
			System.out.println("Pi�ce non plac�e");
		posX = null;
		posY = null;
	}
